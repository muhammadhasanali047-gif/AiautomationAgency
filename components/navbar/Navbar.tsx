  ...
  return (
    <header
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        scrolled
          ? 'bg-slate-900/70 backdrop-blur-xl border-b border-slate-800/80 shadow-sm py-3'
          : 'bg-slate-950/40 backdrop-blur-md py-3.5 sm:py-4 border-b border-white/5'
      }`}
    >
      <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between">
          ...
          <nav className="hidden md:flex items-center gap-0.5 bg-slate-900/50 border border-slate-800/80 rounded-full px-2.5 py-1 backdrop-blur-md">
            {navItems.map((item) => {
                ...
                <Link
                  ...
                  className={`relative px-3 py-1.5 text-xs lg:text-sm transition-colors duration-200 rounded-full ${
                    isActive
                      ...
                      : 'text-slate-400 hover:text-cyan-400 font-medium'
                  }`}
                >
                  {isActive && (
                    <motion.div
                      layoutId="activeNavIndicator"
                      className="absolute inset-0 bg-slate-800/80 border border-slate-700/50 rounded-full shadow-sm -z-10"
          ...
          <div className="hidden md:flex items-center gap-3">
            <Link
              href="/contact"
              className="typography-btn group relative inline-flex items-center gap-2 px-5 py-2.5 rounded-full text-white bg-gradient-to-r from-blue-600 via-indigo-500 to-cyan-500 hover:from-blue-500 hover:to-cyan-400 shadow-[0_0_20px_-5px_rgba(37,99,235,0.4)] hover:shadow-[0_0_25px_-5px_rgba(37,99,235,0.6)] transition-all duration-200 hover:-translate-y-0.5"
            >
          ...
          <div className="md:hidden flex items-center">
            <button
              ...
              className="p-2 rounded-xl text-slate-300 hover:text-cyan-400 hover:bg-slate-800 transition-colors focus:outline-none"
